...
exports.handler = async (event) => {
  ...
  try {
    ...
    // ── GET DEAL: Fetch deal data for pre-filling (uses service key, bypasses RLS) ──
    if (action === 'get_deal') {
      const { deal_id } = body;
      if (!deal_id) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Missing deal_id' }) };
      const deals = await sbFetch(
        `deals?id=eq.${deal_id}&select=*&limit=1`,
        'GET'
      );
      if (deals && deals.length > 0) {
        return { statusCode: 200, headers, body: JSON.stringify({ deal: deals[0] }) };
      }
      return { statusCode: 200, headers, body: JSON.stringify({ deal: null }) };
    }
